...
void CV::rectCustom( float x1, float y1, float x2, float y2, int thick )
{
  //Preenche o quadrado por dentro ate espessura ser o determinado numero de pixels
  for (int i = 0; i < thick; i++)
    CV::rect(x1 + i, y1 + i, x2 - i, y2 - i);

  //Tapa as diagonais que por algum motivo ficam mais claras, ai fica paracendo um envelope
  CV::line(x1, y1, x1 + thick - 1, y1 + thick - 1); //Cima-Esquerda
  CV::line(x2, y1, x2 - thick + 1, y1 + thick - 1); //Cima-Direita
  CV::line(x1, y2, x1 + thick - 1, y2 - thick + 1); //Baixo-Esquerda
  CV::line(x2, y2, x2 - thick + 1, y2 - thick + 1); //Baixo-Direita
}
